...
async function initializeTodaysMoodState() {
  try {
    ...
    if (Array.isArray(todaysMoods) && todaysMoods.length > 0) {
      ...
      const previousMoodValues = todaysMoods.map((moodEntry) => moodEntry?.mood).filter(Boolean);
...
// Only bind homepage-specific listeners when elements exist
  ...
  button.addEventListener('click', function () {
    ...
    if (submitMoodBtn) {
      if (selectedMoods.length >= 1 && selectedMoods.length <= 3) {
        submitMoodBtn.classList.remove('d-none');
      } else {
        submitMoodBtn.classList.add('d-none');
      }
    }
  });
});
  ...
  submitMoodBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      if (hasTodaysMood) {
        ...
        hasTodaysMood = true;
        if (moodBtn) moodBtn.textContent = 'Update your daily mood';
      }
    } catch (error) {
    ...
    document.getElementById('recommendation-message').textContent =
      `Based on how you're feeling, I think you should visit ${town.name}`;
